...
class ModerationValidator:
                    ...
                    for decorator in node.decorator_list:
                        if isinstance(decorator, ast.Call):
                            if (
                                ...
                                hasattr(decorator.func, "attr")
                                and decorator.func.attr == "performance_monitor"
                            ):
    ...
    def validate_auto_moderation(self) -> bool:
        ...
        try:
            ...
            missing_checks = [
                c for c in required_checks if c not in self.results["detection_methods"]
            ]
            if missing_checks:
                self.results["warnings"].append(
                    f"Missing detection methods: {missing_checks}"
                )
    ...
    def check_command_coverage(self):
        ...
        found_commands = [
            cmd for cmd in expected_commands if cmd in self.results["commands"]
        ]
        missing_commands = [
            cmd for cmd in expected_commands if cmd not in self.results["commands"]
        ]

        print(
            f"  ✅ Found: {len(found_commands)}/{len(expected_commands)} expected commands"
        )
        ...
        extra_commands = [
            cmd for cmd in self.results["commands"] if cmd not in expected_commands
        ]
        if extra_commands:
