...
def test_step_5_database():
    ...
    try:
        ...
        # Insert test data
        cursor.execute("INSERT INTO test_performance (name) VALUES (?)", ("performance_test",))
        conn.commit()
    ...
    try:
        ...
        try:
            ...
            async with aiosqlite.connect("test_async.db") as db:
                ...
                """)
                await db.execute("INSERT INTO async_test (data) VALUES (?)", ("async_test",))
                await db.commit()
...
def main():
    ...
    missing_deps = [name for name, available in optional_deps.items() if not available] if optional_deps else []
    if missing_deps:
