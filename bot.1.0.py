...
def get_current_user():
    """Get current user in a container-friendly way"""
    try:
        # Try different methods to get username, with fallbacks for containers
        username = (
            os.environ.get("USER")
        ...
        if username == "container-user":
            try:
                username = os.getlogin()
            except (OSError, AttributeError):
                # This will happen in containers - use fallback
...
class AstraBot(commands.Bot):
    ...
    def setup_logging(self):
        ...
        try:
            ...
            # If we can't write to logs, just continue with console logging
            ...
            try:
                ...
                if config_manager.get("bot_settings.command_sync_on_ready", True):
                    try:
                        ...
                        self.logger.info(
                            f"✅ Application commands synchronized! ({len(synced)} commands)"
                        )
