...
class DiscordRateLimiter:
    ...
    def __init__(self):
        ...
        self.rate_limits = {
            ...
            "messages": {"limit": 5, "window": 5},  # 5 messages per 5 seconds per channel
            "global_messages": {"limit": 50, "window": 60},  # 50 messages per minute globally
            
            # Moderation limits
            "moderation": {"limit": 10, "window": 60},  # 10 moderation actions per minute
            
            # General API limits
            "api_general": {"limit": 30, "window": 60},  # 30 general API calls per minute
        }
    ...
    async def check_rate_limit(self, endpoint: str, identifier: str = "global") -> float:
        """
        ...
        if key in self.backoff_until and current_time < self.backoff_until[key]:
            ...
            logger.warning(f"⏸️  Rate limiter: backoff active for {key}, delay: {delay:.1f}s")
            return delay
    ...
    def _update_global_message_count(self):
        ...
        if self.global_message_count >= 50:  # Global limit
            logger.warning(f"🚨 Approaching global message limit: {self.global_message_count}/50")
    ...
    def handle_discord_rate_limit(self, retry_after: float, endpoint: str = "messages", identifier: str = "global"):
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self.stats,
            "active_backoffs": len([k for k, v in self.backoff_until.items() if time.time() < v]),
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if hasattr(e, 'status') and e.status == 429:
                    ...
                    discord_rate_limiter.handle_discord_rate_limit(retry_after, endpoint, identifier)
                raise
