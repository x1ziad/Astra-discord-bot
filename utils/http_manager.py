...
class EnhancedHTTPManager:
    ...
    def _check_rate_limit(self, service: str) -> bool:
        ...
        if (rate_limit.current_minute_count >= rate_limit.requests_per_minute or
            rate_limit.current_hour_count >= rate_limit.requests_per_hour):
    ...
    async def request(self, 
        ...
        if use_cache:
            ...
            if cache_key in self._cache:
                ...
                if (datetime.utcnow() - self._cache_timestamps[cache_key]).total_seconds() < self.cache_ttl:
        ...
        if not self._check_rate_limit(service):
            rate_limit = self.rate_limits[service]
            wait_time = 61 - (datetime.utcnow() - rate_limit.minute_reset).total_seconds()
        ...
        try:
            async with self._session.request(method, url, **kwargs) as response:
                ...
                if (use_cache and cache_key and 
                    response.status == 200 and method.upper() == 'GET'):
                    try:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': ((self.stats.requests_made - self.stats.failed_requests) / 
    ...
    def configure_rate_limit(self, service: str, requests_per_minute: int = 60, 
                           requests_per_hour: int = 1000):
        ...
        logger.info(f"Rate limit configured for {service}: {requests_per_minute}/min, {requests_per_hour}/hour")
