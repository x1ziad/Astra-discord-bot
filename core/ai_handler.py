...
class AIHandler:
    ...
    async def _should_respond(self, message: discord.Message) -> bool:
        ...
        if any(word in content for word in [
            "astra", "hey", "hello", "help", "hi",
            "مرحبا", "salut", "bonjour", "hallo", "hola"
        ]):
            return True
        ...
        if PERSONALITY_INTEGRATION_AVAILABLE:
            try:
                ...
                if personality_response:
                    logger.info(f"🎭 Personality response generated for identity question")
    ...
    async def _get_fallback_response(self, message: discord.Message, language: str = 'english') -> str:
        ...
        if any(word in content for word in ['who are you', 'what are you', 'من أنت', 'qui êtes-vous', 'wer bist du']):
            return lang_responses['who']
        elif any(word in content for word in ['hello', 'hi', 'مرحبا', 'salut', 'bonjour', 'hallo']):
        ...
        elif any(word in content for word in ['thanks', 'thank', 'شكرا', 'merci', 'danke']):
