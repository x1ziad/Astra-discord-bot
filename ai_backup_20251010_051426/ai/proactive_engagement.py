...
class ProactiveEngagement:
    ...
    def __init__(self):
        ...
        self.user_engagement_history: Dict[int, List[datetime]] = (
            {}
    ...
    async def should_engage_proactively(
        ...
        topic_score, topic_reasons = await self._analyze_topic_interest(
            message_lower, user_profile
        )
        engagement_score += topic_score
        reasons.extend(topic_reasons)

        # 2. Emotional context engagement
        emotion_score, emotion_reasons = await self._analyze_emotional_context(
            message_lower
        )
    ...
    async def _track_user_engagement(self, user_id: int):
        ...
        if len(self.user_engagement_history[user_id]) > 50:
            self.user_engagement_history[user_id] = self.user_engagement_history[
                user_id
            ][-50:]

    async def _analyze_topic_interest(
        self, message_lower: str, user_profile: Dict = None
        ...
        if user_profile and "user_personality" in user_profile:
            favorite_topics = user_profile["user_personality"].get(
                "favorite_topics", []
            )
    ...
    async def _analyze_help_seeking(
        self, message_lower: str
    ) -> Tuple[float, List[str]]:
    ...
    async def _analyze_personal_engagement(
        self, user_id: int, user_profile: Dict = None
    ) -> Tuple[float, List[str]]:
        ...
        total_interactions = user_profile.get("interaction_history", {}).get(
        ...
        positive_feedback = user_profile.get("interaction_history", {}).get(
            "positive_feedback", 0
        )
        total_feedback = positive_feedback + user_profile.get(
            "interaction_history", {}
        ).get("negative_feedback", 0)
        ...
        if user_id in self.user_engagement_history:
            recent_engagements = [
                eng
                for eng in self.user_engagement_history[user_id]
    ...
    async def _analyze_conversation_context(
        self, message_content: str, channel_id: int
    ) -> Tuple[float, List[str]]:
    ...
    async def generate_engagement_type(
        self, message_content: str, engagement_reason: str, user_profile: Dict = None
    ) -> str:
