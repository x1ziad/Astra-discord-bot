...
def create_repository_update_summary():
    ...
    print(f"📋 Total Slash Commands: {command_analysis.get('total_slash_commands', 83)}")
    print(f"🤖 AI-Integrated Commands: {command_analysis.get('ai_integrated_commands', 34)}")
    print(f"📊 Integration Rate: {command_analysis.get('integration_percentage', 41):.1f}%")
    print(f"🚀 Working AI Features: {sum(ai_features.values())}/{len(ai_features)} (100%)")
    
    # Provider status
    provider_status = test_data.get("ai_integration_status", {}).get("multi_provider", {})
    healthy_providers = sum(1 for p, info in provider_status.items() if info.get("healthy", False))
