...
def test_personality_system():
    ...
    try:
        ...
        print(f"🎪 High playfulness scenario: playful={personality.playful}, creative={personality.creative}")
        
        # Test high analytical scenario  
        personality.analytical = 0.9
        personality.curious = 0.8
        personality.assertive = 0.7
        print(f"🔬 High analytical scenario: analytical={personality.analytical}, curious={personality.curious}")
            ...
            for trait, value in dominant_traits:
                if value > 0.7:
                    if trait == 'analytical':
                        ...
                        instructions.append("Show warm understanding and emotional connection")
                    elif trait == 'curious':
                        instructions.append("Ask follow-up questions and show genuine interest")
                    elif trait == 'creative':
                        instructions.append("Use space metaphors and imaginative language")
                    elif trait == 'supportive':
                        instructions.append("Be encouraging and motivational")
                    elif trait == 'playful':
                        instructions.append("Include humor, puns, and light-hearted responses")
                    elif trait == 'assertive':
                        instructions.append("Be confident and direct with clear opinions")
                    elif trait == 'adaptable':
                        instructions.append("Adjust style to match conversation needs")
            return " | ".join(instructions) if instructions else "Maintain balanced responses"
            ...
            for trait, value in dominant_traits[:3]:
                if value > 0.7:
                    if trait == 'analytical':
                        ...
                        preview = "Warm, understanding responses with emotional connection"
                    else:
        ...
        enhanced_message = enhance_message_with_personality(test_message, personality, dominant_traits)
...
def test_commands_integration():
    ...
    print("  • /quick_personality <trait> <value> - Quick trait adjustment with immediate test")
