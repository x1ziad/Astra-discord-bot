    ...
    try:
        ...
        for message in test_messages:
            mood, confidence, emotions = (
                engine.sentiment_analyzer.analyze_emotional_state(message)
            )
        ...
        for message in [
            ...
            response = await engine.process_conversation(
                message=message, user_id=test_user_id
            )
        ...
        print(
            "Some AI dependencies may not be installed, but the bot will still work with fallbacks!"
        )
        return False
...
async def main():
    ...
    if ai_success:
        ...
        print(
            "⚠️ Some advanced features may use fallbacks, but core functionality works!"
        )
