    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(
                "https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY", timeout=10
            ) as response:
                if response.status == 200:
                    ...
                    print(
                        f"📝 Description: {data.get('explanation', 'No description')[:150]}..."
                    )
        ...
        try:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"⏰ Last Updated: {timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                    )
