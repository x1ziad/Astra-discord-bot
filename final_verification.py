...
def main():
        ...
        if commands:
            ...
            for i, (cmd_name, cmd_desc) in enumerate(commands, 1):
                ...
                if cog_name == 'nexus':
                    ...
                    prefix = f'/{cog_name}' if not cmd_name.startswith(cmd_name) else '/'
    ...
    # Breakdown by cog
    print(f"\n📋 Commands by Module:")
    for cog_name, commands in sorted(cog_commands.items(), key=lambda x: len(x[1]), reverse=True):
    ...
    print(f"🚀 System Status: {'FULLY OPERATIONAL' if total_commands > 30 else 'NEEDS REVIEW'}")
    print("="*100)
