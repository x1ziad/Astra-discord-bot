...
def debug_ai_credentials():
    ...
    try:
        ...
        if not any([credentials_status['OPENAI_API_KEY'], credentials_status['OPENROUTER_API_KEY']]):
            critical_missing.append('AI_API_KEY (OpenAI or OpenRouter)')
        
        if critical_missing:
            logger.warning(f"⚠️  Critical credentials missing: {', '.join(critical_missing)}")
        else:
