...
class AstraDemoBot:
    ...
    async def simulate_user_interaction(
        self, username: str, message: str, guild_name: str = "Demo Server"
    ):
        ...
        context = {
            ...
            "guild_id": 12345,
            "is_repeat_user": any(
                h["username"] == username for h in self.conversation_history
            ),
            "message_count": len(
                [h for h in self.conversation_history if h["username"] == username]
            )
            + 1,
        }
        ...
        response, metadata = await self.optimizer.optimize_request(
        ...
        print(
            f"⚡ Response time: {response_time:.3f}s | Type: {metadata.get('type')} | Context: {metadata.get('context_aware', False)}"
        )
    ...
    async def show_performance_summary(self, interactions):
        ...
        avg_response_time = (
            sum(i["response_time"] for i in interactions) / total_interactions
        )
        ...
        print(
            f"   Context-Aware Responses: {context_aware}/{total_interactions} ({context_aware/total_interactions*100:.1f}%)"
        )
        print(
            f"   Pattern Matches: {pattern_matches}/{total_interactions} ({pattern_matches/total_interactions*100:.1f}%)"
        )
        print(
            f"   Quick Responses: {quick_responses}/{total_interactions} ({quick_responses/total_interactions*100:.1f}%)"
        )
        print()

        print("⚡ Speed Distribution:")
        print(
            f"   Under 1ms: {under_1ms}/{total_interactions} ({under_1ms/total_interactions*100:.1f}%)"
        )
        print(
            f"   Under 10ms: {under_10ms}/{total_interactions} ({under_10ms/total_interactions*100:.1f}%)"
        )
        print(
            f"   Under 100ms: {under_100ms}/{total_interactions} ({under_100ms/total_interactions*100:.1f}%)"
        )
        print()
    ...
    async def demo_cache_performance(self):
        ...
        start_time = time.time()
        response1, meta1 = await self.optimizer.optimize_request(
            test_message, 9999, {"username": test_user}
        )
        ...
        await self.optimizer.cache_response(
            test_message, cached_response, 9999, {"username": test_user}
        )
        ...
        start_time = time.time()
        response2, meta2 = await self.optimizer.optimize_request(
            test_message, 9999, {"username": test_user}
        )
        ...
        if first_time > 0:
            ...
            speed_multiplier = (
                first_time / cached_time if cached_time > 0 else float("inf")
            )
            print(f"\n📈 Cache Performance:")
