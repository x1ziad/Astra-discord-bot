...
class Quiz(commands.GroupCog, name="quiz"):
    ...
    def __init__(self, bot):
        ...
        self.logger.info(
            f"Quiz cog initialized with {sum(len(v) for v in self.questions.values())} questions"
        )
    ...
    def _load_questions(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            if custom_questions_file.exists():
                ...
                self.logger.info(
                    f"Loaded custom questions: {sum(len(v) for v in custom_questions.values())} questions"
                )
    ...
    def update_user_stats(
        self, user_id: int, guild_id: int, correct: bool, points: int, category: str
    ):
    ...
    @app_commands.command(
        name="quiz_leaderboard", description="View the quiz leaderboard"
    )
    ...
    async def leaderboard_command(
        self, interaction: discord.Interaction, sort_by: Optional[str] = "points"
    ):
        ...
        leaderboard_view = LeaderboardView(
            self.leaderboard[guild_id], sort_by or "points"
        )
    ...
    @app_commands.command(
        name="categories", description="View available quiz categories"
    )
    ...
    async def categories_command(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            ...
            + "\n".join(
                [f"• `/quiz start category:{cat}`" for cat in self.questions.keys()]
            )
    ...
    @app_commands.command(
        name="add", description="Add a new quiz question (Admin only)"
    )
    @app_commands.checks.cooldown(1, 10)
    # @feature_enabled("quiz_system")
    ...
    async def add_question(self, interaction: discord.Interaction):
        ...
        if (
            ...
            and not self.config.has_role_permission(interaction.user, "quiz_master")
        ):
            await interaction.response.send_message(
                "❌ You need Quiz Master permissions to add questions.", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        name="reset", description="Reset quiz leaderboard (Admin only)"
    )
        ...
        class ConfirmView(discord.ui.View):
            ...
            async def confirm(
                ...
                await confirm_interaction.response.send_message(
                    "✅ Leaderboard has been reset!", ephemeral=True
                )
            ...
            async def cancel(
                ...
                await cancel_interaction.response.send_message(
                    "❌ Leaderboard reset cancelled.", ephemeral=True
                )
