...
Analyze your capabilities, performance, and potential. What are your strengths? What could be improved? What makes you unique? Be self-aware and analytical but confident."""
        ...
        try:
            ...
            if diagnostics["recommendations"]:
                embed.add_field(
                    ...
                    value="\n".join(
                        [f"• {rec}" for rec in diagnostics["recommendations"][:5]]
                    ),
                    inline=False,
                )
    ...
    async def _perform_ai_diagnostics(self) -> Dict[str, Any]:
        ...
        # Analyze bot status
        loaded_cogs = list(self.bot.cogs.keys())
        total_extensions = len(loaded_cogs) + len(
            getattr(self.bot, "failed_extensions", {})
        )
        ...
        if not critical_ok:
            recommendations.append(
                "Critical system modules missing - check extension loading"
            )
        if len(getattr(self.bot, "failed_extensions", {})) > 0:
            recommendations.append(
                f"{len(self.bot.failed_extensions)} extensions failed to load"
            )
        ...
        return {
            "overall_status": (
                ...
                else "🟡 Good" if cpu_usage < 80 else "🔴 Needs Attention"
            ),
    ...
    async def capability_scan(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if capabilities["unique_features"]:
                embed.add_field(
                    ...
                    value="\n".join(
                        [
                            f"• {feature}"
                            for feature in capabilities["unique_features"][:6]
                        ]
                    ),
                    inline=False,
                )
            ...
            awareness_level = (
                ...
                if capabilities["self_awareness_score"] > 85
                else "🤖 **Standard AI Awareness**"
            )
            embed.add_field(
    ...
    async def _analyze_bot_capabilities(self) -> Dict[str, Any]:
        ...
        self_awareness_score = min(
            100, 70 + (ai_enhanced * 2) + (len(unique_features) * 2)
        )
        ...
        return {
            ...
            "security": any(
                "security" in str(cog).lower() or "moderation" in str(cog).lower()
