...
class EnhancedAdmin(commands.GroupCog, name="admin"):
    ...
    async def system_command(self, interaction: discord.Interaction):
        ...
        if (current_time - self._cache_time) < self._cache_duration and self._system_info_cache:
    ...
    async def ai_control_command(
        self, 
        interaction: discord.Interaction, 
        action: Literal["status", "restart", "clear_cache", "switch_provider", "health_check"],
        provider: Optional[Literal["universal", "openrouter", "github", "openai"]] = None
    ):
            ...
            if action == "status":
                ...
                if hasattr(ai_client, 'get_health_status'):
                    ...
                    embed = discord.Embed(
                        ...
                        color=0x00FF00 if status.get('status') == 'healthy' else 0xFF6B6B,
                        timestamp=datetime.now(timezone.utc)
                    )
                    ...
                    await interaction.followup.send("❌ AI client doesn't support health status checks.")
                ...
                # Test database
                try:
                    if hasattr(ai_client, 'db_path') and os.path.exists(ai_client.db_path):
                ...
                await interaction.followup.send(f"❌ Action '{action}' not yet implemented.")
    ...
    @app_commands.describe(
        lines="Number of log lines to show (default: 20, max: 100)"
    )
        ...
        try:
            if action == "show":
                ...
                for i in range(0, len(config_items), chunk_size):
                    ...
                    field_name = f"Configuration ({i+1}-{min(i+chunk_size, len(config_items))})"
                    embed.add_field(
                        name=field_name,
                        value="\n".join(chunk),
                        inline=False
                ...
                if not key:
                    await interaction.followup.send("❌ Key parameter required for 'get' action.")
                    return
                ...
                if not key or value is None:
                    await interaction.followup.send("❌ Both key and value parameters required for 'set' action.")
                    return
