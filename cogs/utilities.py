...
class Utilities(commands.Cog):
    ...
    async def serverinfo_command(self, interaction: discord.Interaction):
        ...
        if total_members:
            ...
            embed.add_field(
                name="👥 Members", value="Member count unavailable", inline=True
            )
    ...
    async def timestamp_command(
        ...
        try:
            ...
            if time_input.lower() == "now":
                ...
                match = re.match(
                    r"(\d+)\s*(minute|hour|day|week|month|year)s?", time_str.lower()
                )
                if not match:
                ...
                for fmt in formats_to_try:
                    try:
                        target_time = datetime.strptime(time_input, fmt).replace(
                            tzinfo=timezone.utc
                        )
                        break
                    except ValueError:
            ...
            embed.add_field(
                name="📋 All Format Examples", value="\n".join(examples), inline=False
            )
    ...
    async def remind_command(
        ...
        try:
            ...
            if not match:
                raise ValueError(
                    "Invalid time format. Use format like '5m', '2h', '1d'"
                )
            ...
            await interaction.response.send_message(
                f"❌ **Error setting reminder:** {str(e)}", ephemeral=True
            )
