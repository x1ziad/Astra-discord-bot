...
class AdvancedAICog(commands.Cog):
    ...
    async def _track_channel_activity(self, message: discord.Message):
        ...
        activity["recent_messages"] = [
            msg for msg in activity["recent_messages"] 
            if msg["timestamp"] > cutoff
        ]
        activity["recent_topics"] = activity["recent_topics"][-20:]  # Keep last 20 topics
    ...
    async def _should_ai_respond(self, message: discord.Message) -> bool:
        ...
        if user_id in self.conversation_cooldowns:
            if datetime.utcnow() - self.conversation_cooldowns[user_id] < timedelta(seconds=3):
                return False
        ...
        space_keywords = ["space", "star", "planet", "galaxy", "cosmos", "universe", "astronomical"]
        stellaris_keywords = ["stellaris", "empire", "species", "galactic", "federation", "hyperlane", "ascension"]
        science_keywords = ["science", "research", "discovery", "theory", "experiment", "analysis"]
        tech_keywords = ["ai", "artificial intelligence", "technology", "quantum", "computer"]
        ...
        emotional_keywords = ["sad", "frustrated", "confused", "lost", "don't understand", "struggling"]
        if any(keyword in content_lower for keyword in emotional_keywords):
            return True
        
        # Celebration detection
        celebration_keywords = ["amazing", "awesome", "incredible", "fantastic", "achieved", "success"]
    ...
    async def _process_ai_conversation(self, message: discord.Message):
        ...
        try:
            ...
            # Log interaction
            self.logger.info(f"AI response generated for user {user_id} in {response_time:.2f}s")
        ...
        try:
            ...
            for channel in guild.text_channels:
                ...
                if timedelta(minutes=30) < time_since_activity < timedelta(hours=2):
                    ...
                    for user_id in list(channel_activity["active_users"])[-5:]:  # Recent active users
                        if await self.conversation_engine.should_proactively_engage(
                            user_id, channel_activity
                        ):
                            await self._initiate_proactive_conversation(channel, user_id)
                            break  # Only engage with one user per check
        
        except Exception as e:
            self.logger.error(f"Proactive engagement check error: {e}")
    
    async def _initiate_proactive_conversation(self, channel: discord.TextChannel, user_id: int):
        ...
        try:
            ...
            for user_id in list(self.engagement_patterns.keys()):
                ...
                self.engagement_patterns[user_id] = [
                    timestamp for timestamp in self.engagement_patterns[user_id]
                    if timestamp > cutoff
                ]
    ...
    async def conversation_cleanup_task(self):
        ...
        try:
            ...
            expired_cooldowns = [
                user_id for user_id, timestamp in self.conversation_cooldowns.items()
            ...
            expired_joins = [
                user_id for user_id, timestamp in self.user_join_timestamps.items()
    ...
    @app_commands.command(name="ai_stats", description="View AI conversation statistics")
    async def ai_stats(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if popular_topics:
                topics_text = "\n".join([f"{topic}: {count}" for topic, count in popular_topics])
                embed.add_field(
                    name="🔥 Popular Topics",
                    value=topics_text,
                    inline=True
    ...
    @app_commands.command(name="ai_personality", description="View or modify AI personality settings")
    async def ai_personality(self, interaction: discord.Interaction, trait: str = None, value: str = None):
        ...
        try:
            if not trait:
                ...
                embed.add_field(
                    name="🌟 Core Traits",
                    value="\n".join(f"• {trait}" for trait in personality['core_traits']),
                    inline=False
                )
                ...
                if not interaction.user.guild_permissions.administrator:
                    await interaction.response.send_message("You need administrator permissions to modify AI personality.", ephemeral=True)
                    return
                
                # Implementation for personality modification would go here
                await interaction.response.send_message(f"Personality modification for '{trait}' is not yet implemented.", ephemeral=True)
        
        except Exception as e:
            self.logger.error(f"AI personality command error: {e}")
            await interaction.response.send_message("Error accessing personality settings.", ephemeral=True)
    
    @app_commands.command(name="ai_reset", description="Reset AI conversation history for yourself")
    async def ai_reset(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message("Error resetting conversation history.", ephemeral=True)
    
    @app_commands.command(name="ai_mood", description="Analyze the mood of the conversation")
    async def ai_mood_analysis(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Mood visualization
            mood_emojis = {
                "ecstatic": "🤩", "excited": "🎉", "happy": "😊", "content": "😌",
                "curious": "🤔", "neutral": "😐", "pensive": "🤔", "confused": "😕",
                "concerned": "😟", "frustrated": "😤", "sad": "😢", "angry": "😡"
            }
            ...
            await interaction.response.send_message("Error analyzing conversation mood.", ephemeral=True)
    
    @app_commands.command(name="ai_topics", description="View trending conversation topics")
        ...
        try:
            ...
            if popular_topics:
                ...
                for i, (topic, count) in enumerate(popular_topics[:10], 1):
                    ...
                    topic_text.append(f"{emoji} **{topic.replace('_', ' ').title()}** ({count} mentions)")
                
                embed.description = "\n".join(topic_text)
            else:
                embed.description = "No topics have been discussed yet. Start a conversation! 🚀"
            ...
            await interaction.response.send_message("Error retrieving topic information.", ephemeral=True)
    
    @app_commands.command(name="ai_personality", description="Customize AI personality for this server")
    @app_commands.describe(
        trait="Personality trait to adjust",
        level="Level from 1-10 (10 being maximum)"
    )
    async def ai_personality_customize(self, interaction: discord.Interaction, 
                                     trait: str = None, level: int = None):
        ...
        try:
            ...
            if not trait:
                ...
                self.logger.info(f"Personality trait {trait} set to {level} by {interaction.user.id}")
            ...
            await interaction.response.send_message("Error customizing personality.", ephemeral=True)
    
    @app_commands.command(name="ai_engage", description="Manually trigger proactive AI engagement")
    @app_commands.describe(user="User to engage with (admin only)")
    async def manual_engage(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        try:
            ...
            self.logger.info(f"Manual AI engagement triggered for user {target_user.id}")
            
        except Exception as e:
            self.logger.error(f"Manual engage error: {e}")
            await interaction.response.send_message("Error triggering AI engagement.", ephemeral=True)
