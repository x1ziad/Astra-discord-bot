...
class AdvancedAICog(commands.Cog):
    ...
    def _setup_ai_client(self):
        """Setup AI engine with fallback options and performance optimization"""
        try:
            ...
            for client_name, client_factory in ai_client_options:
                try:
            ...
            self.logger.error(f"Critical error in AI client setup: {e}")
            self.ai_client = None

    def _initialize_universal_ai_client(self):
        ...
        try:
            ...
            if user_id in self.conversation_history:
                ...
                if len(user_history) > 0:
                    ...
                    message_types = [
                        msg.get("message_type", "casual") for msg in recent_messages
                    ]

                    conversation_context = {
                        "active_conversation": len(recent_messages) > 0,
                        "conversation_tone": (
                            max(set(message_types), key=message_types.count)
                            if message_types
                            else "casual"
                        ),
                        "user_engagement": len(recent_messages),
                        "last_interaction": (
                            user_history[-1].get("message_type", "casual")
                            if user_history
                            else "new"
                        ),
                        "in_dm": (
                            user_history[-1].get("is_dm", False)
                            if user_history
                            else False
                        ),
                    }
            ...
            if conversation_context.get("active_conversation"):
                ...
                if tone == "question":
                    enhanced_prompt = (
                        f"Continue our Q&A conversation naturally: {prompt}"
                    )
                elif tone == "technical":
                    enhanced_prompt = f"Continue our technical discussion: {prompt}"
                elif tone == "emotional":
                    enhanced_prompt = (
                        f"Respond empathetically, continuing our conversation: {prompt}"
                    )
                elif tone == "sharing":
                    enhanced_prompt = (
                        f"Respond to their story/sharing conversationally: {prompt}"
                    )
                else:
            ...
            if hasattr(self.ai_client, "generate_response"):
                try:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Context-aware fallback timeout: {fb_error}"
                        )
                        response = None
            ...
            if not response:
                try:
                    enhanced_context = {
                        ...
                        "conversation_active": conversation_context.get(
                            "active_conversation", False
                        ),
                        "conversation_tone": conversation_context.get(
                ...
                self.conversation_history[user_id].extend(
                    [
                        ...
                        },
                        {
                            ...
                            "timestamp": time.time(),
                            "context_used": bool(
                                conversation_context.get("active_conversation")
                            ),
                        },
                    ]
                )
...
- Conversational, witty, and genuinely helpful
- Space/sci-fi enthusiast with gaming knowledge (especially Stellaris)
- Uses metaphors naturally without being overwhelming
- Adapts tone to user's emotional state and conversation context
- Balances professionalism with warmth and humor"""

        # OPTIMIZATION: Context-specific personality adaptations
        if conversation_context.get("conversation_tone") == "technical":
            return f"{base_personality}\n- Focus on technical accuracy and detailed explanations\n- Use precise language while maintaining friendliness"
        elif conversation_context.get("conversation_tone") == "emotional":
            return f"{base_personality}\n- Prioritize empathy and emotional support\n- Use gentle, understanding language with appropriate responses"
        elif conversation_context.get("active_conversation"):
            return f"{base_personality}\n- Continue conversation naturally with contextual awareness\n- Reference previous topics when relevant"
        else:
            return f"{base_personality}\n- Engage with fresh energy and enthusiasm\n- Be welcoming and establish positive rapport"

    def _optimize_prompt_for_speed(self, prompt: str, context: Dict) -> str:
        """Optimize prompts for faster AI processing while maintaining quality"""
    ...
    # /chat command removed - Focusing on natural conversation flow
    # Users can now simply mention @Astra or talk naturally in chat!
    ...
    async def _lightning_analyze_message(self, message: discord.Message):
        ...
        try:
            ...
            should_store = (
                content_words > 2  # Substantial messages
                or not is_reactive  # Non-reactive short messages
                or len(self.conversation_history[message.author.id])
                > 0  # Active conversation
            )
            ...
            if should_store:
                ...
                message_data = {
                    ...
                    "content": message.content[
                        :200
                    ],  # Increased from 100 for better context
                    "timestamp": time.time(),
    ...
    def _classify_message_type(self, content: str) -> str:
        ...
        if "?" in content:
            ...
            for tech in ["stellaris", "game", "code", "tech", "computer"]
        ):
            return "technical"
    ...
    async def _lightning_determine_interaction(
        ...
        try:
            ...
            if (
                any(keyword in content for keyword in sharing_keywords)
                and len(content_words) > 4
            ):
                interaction_score += 30
                interaction_type = "story"

            # PRIORITY 6: Technical/gaming discussions (Stellaris focus)
            tech_gaming_keywords = [
                "stellaris",
                "game",
                "gaming",
                "tech",
            ...
            if len(user_history) > 0:
                ...
                if any(
                    indicator in recent_content.lower()
            ...
            return {
                ...
                "response_style": (
                    ...
                    else "helpful" if interaction_score > 40 else "casual"
                ),
                "conversation_context": len(user_history) > 0,
                "message_quality": (
                    "high"
                    if message_length > 10
                    else "medium" if message_length > 3 else "low"
                ),
            }
        ...
        try:
            ...
            if interaction_type == "direct":
                ...
                if optimization_meta.get("type") == "quick_response":
                    ...
                    context_prompt = (
                        f"Answer helpfully and conversationally: {optimized_prompt}"
                    )
                ...
                if any(emotion in content_lower for emotion in positive_emotions):
                    ...
                    responses = [
                        ...
                        "Hang in there! 💙",
                        "That's frustrating, but you've got this! 💪",
                        "Rough times don't last, but resilient people do! ✨",
                ...
                story_responses = [
                    ...
                    "That's fascinating! How did that go? 🌟",
                ...
                tech_responses = [
                    ...
                    "Ah, a fellow tech enthusiast! 💻",
                    "Stellaris? You've got excellent taste! ⭐",
                    "Gaming wisdom incoming! 🎮",
                    "Tech talk is the best talk! 🔧",
                    "Ready to dive deep into this! 🚀",
                ]
                ...
                opinion_responses = [
                    ...
                    "Ooh, philosophical territory! What do you think? 🧠",
                    "Debates are the spice of conversation! 🌶️",
                ...
                should_use_ai = (
                    decision.get("message_quality") == "high"  # Quality messages
                    or decision.get("conversation_context")  # Active conversation
                    or len(message.content.split()) > 8  # Substantial messages
                    or random.random() > 0.5  # 50% chance for better engagement
                )

                if should_use_ai:
                    # Enhanced prompt for better context awareness
                    context_prompt = (
                        f"Respond naturally and conversationally to: {optimized_prompt}"
                    )
                    if decision.get("conversation_context"):
            ...
            # Graceful fallback
            fallback_responses = [
                "Hmm, something went sideways there! 🤖",
                "My circuits got a bit tangled! Try again? ⚡",
    ...
    async def personality_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="🧠 AstraBot Personality Status",
                description=f"**Current Mode:** {current_mode.replace('_', ' ').title()}\n\nHere's my current personality configuration:",
                color=0x7289DA,
                timestamp=datetime.now(timezone.utc),
            )
            ...
            embed.add_field(
                ...
                value=f"**Humor:** {params['humor']}%\n"
                f"**Honesty:** {params['honesty']}%\n"
                f"**Formality:** {params['formality']}%\n"
                f"**Empathy:** {params['empathy']}%",
                inline=True,
            )

            embed.add_field(
                name="⚡ Behavioral Traits",
                value=f"**Strictness:** {params['strictness']}%\n"
                f"**Initiative:** {params['initiative']}%\n"
                f"**Transparency:** {params['transparency']}%",
                inline=True,
            )
            ...
            embed.add_field(
                ...
                f"**Version:** 2.0.0\n"
                f"**Creator:** <@7zxk>\n"
                f"**Launch Date:** October 5th, 2025",
                inline=True,
            )

            # Mode descriptions
            mode_descriptions = {
                "security": "Security-focused, vigilant and protective",
                "social": "Friendly, engaging and conversational",
                "developer": "Technical, precise and code-oriented",
                "mission_control": "Professional, authoritative and strategic",
                "adaptive": "Dynamically adjusting to context and needs",
            }

            embed.add_field(
                name="� Current Mode",
                value=f"**{current_mode.replace('_', ' ').title()}**\n{mode_descriptions.get(current_mode, 'Custom personality configuration')}",
                inline=True,
            )
            ...
            embed.add_field(
                name="📊 System Status",
                value=f"**Personality System:** ✅ Active\n"
                f"**AI Integration:** ✅ Functional\n"
                f"**Response Adaptation:** ✅ Enabled\n"
                f"**Self-Awareness Level:** Advanced 🚀",
                inline=False,
            )
