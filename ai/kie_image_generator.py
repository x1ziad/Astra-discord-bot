"""
Kie.AI Image Generator for Astra Bot
High-quality image generation using Kie.AI API
"""
...
# Set up logging
logger = logging.getLogger("astra.kie_image")
...
class KieAIImageConfig:
    """Configuration for Kie.AI image generation"""

    def __init__(self):
        self.api_key = os.getenv("KIE_AI_API_KEY")
        self.base_url = "https://api.kie.ai/v1"
        self.model = "flux-dev"  # Kie.AI's high-quality model
        self.max_retries = 3
        self.retry_delay = 2.0
        self.timeout = 120.0  # Kie.AI can take longer
...
class KieAIImageGenerator:
    """
    Kie.AI-based image generation system
    High-quality image generation with comprehensive error handling
    """

    def __init__(self, config: Optional[KieAIImageConfig] = None):
        self.config = config or KieAIImageConfig()
        self.logger = logging.getLogger("astra.kie_image")
        self.session = None
    ...
    def _initialize_client(self) -> bool:
        ...
        try:
            if not self.config.api_key:
                self.logger.error("❌ KIE_AI_API_KEY environment variable not set")
                return False

            self.is_initialized = True
            self.logger.info("✅ Kie.AI Image Generator initialized successfully")
            self.logger.info(
            ...
            self.logger.error(f"❌ Failed to initialize Kie.AI client: {e}")
            self.is_initialized = False
            return False
        ...
        try:
            ...
            if not self.is_available():
                return ImageGenerationResult(
                    success=False,
                    error="Kie.AI Image Generator not available",
            ...
            result = await self._call_kie_api(final_prompt, request)
    ...
    async def _call_kie_api(
        ...
        """Make the actual API call to Kie.AI"""
        ...
        # Prepare request payload for Kie.AI
        payload = {
            "prompt": prompt,
            "model": self.config.model,
            "width": width,
        ...
        try:
            # Make API request
            url = f"{self.config.base_url}/images/generate"

            self.logger.info(f"🌐 Making request to Kie.AI: {url}")

            async with session.post(url, json=payload, headers=headers) as response:
                if response.status != 200:
                    error_text = await response.text()
                    self.logger.error(
                        f"❌ Kie.AI API error {response.status}: {error_text}"
                    )
                    return ImageGenerationResult(
                        success=False,
                ...
                self.logger.info(f"📦 Kie.AI response: {response_data}")
                ...
                if not image_url:
                    # Try base64 data if URL not available
                    ...
                    if base64_data:
                        try:
                            ...
                            return ImageGenerationResult(
                                ...
                                text_response=f"Generated with Kie.AI {self.config.model}",
                            )
                        except Exception as e:
                            return ImageGenerationResult(
                                success=False,
                    ...
                    return ImageGenerationResult(
                        success=False,
                        error="No valid image URL or data in response",
            ...
            self.logger.error(f"❌ Kie.AI API call failed: {e}")
            return ImageGenerationResult(
                success=False,
    ...
    async def test_connection(self) -> Dict[str, Any]:
        """Test the connection to Kie.AI API"""
        try:
            if not self.is_available():
                return {
                    "success": False,
...
# Global instance
_global_generator: Optional[KieAIImageGenerator] = None


def get_kie_generator() -> KieAIImageGenerator:
    """Get the global Kie.AI image generator instance"""
    global _global_generator
    if _global_generator is None:
        _global_generator = KieAIImageGenerator()
...
async def main():
    """Example usage of the Kie.AI Image Generator"""
    # Test the generator
    generator = get_kie_generator()

    print("🧪 Testing Kie.AI Image Generator...")
    ...
    if test_result.get("success"):
        ...
        if result.success and result.image_data:
            ...
            image.save("test_kie_image.png")
            print(f"✅ Image saved as test_kie_image.png")
