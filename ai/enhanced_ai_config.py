...
class AIModelConfig:
    ...
    def get_efficiency_score(self) -> float:
        ...
        return (self.quality_score * self.speed_score) / max(self.cost_per_1k_tokens, 0.001)
...
class EnhancedAIConfig:
    ...
    def _load_configuration(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "model_selection_strategy": os.getenv("MODEL_SELECTION_STRATEGY", "efficiency"),  # efficiency, quality, speed, cost
            "auto_model_switching": os.getenv("AUTO_MODEL_SWITCHING", "true").lower() == "true",
            "quality_threshold": float(os.getenv("QUALITY_THRESHOLD", "0.7")),
            
            # Token optimization
            "max_tokens": int(os.getenv("AI_MAX_TOKENS", "1000")),
            "temperature": float(os.getenv("AI_TEMPERATURE", "0.7")),
            "token_optimization": os.getenv("TOKEN_OPTIMIZATION", "true").lower() == "true",
            ...
            "conversation_memory_optimization": os.getenv("CONVERSATION_MEMORY_OPTIMIZATION", "true").lower() == "true",
            "context_window_optimization": os.getenv("CONTEXT_WINDOW_OPTIMIZATION", "true").lower() == "true",
            "user_personalization": os.getenv("USER_PERSONALIZATION", "true").lower() == "true",
    ...
    def _validate_configuration(self, config: Dict[str, Any]):
        ...
        if not config.get("ai_api_key"):
            self.logger.warning("⚠️ AI_API_KEY not configured - AI features will be limited")
        
        if config.get("max_tokens", 0) > 2000:
            self.logger.warning(f"⚠️ max_tokens ({config['max_tokens']}) is high - consider reducing for cost optimization")
        
        if config.get("temperature", 0) > 1.0:
            self.logger.warning(f"⚠️ temperature ({config['temperature']}) is above 1.0 - may cause unpredictable responses")
    ...
    def get_model_config(self, provider: AIProviderType) -> AIModelConfig:
        ...
        return self.model_configs.get(provider, self.model_configs[AIProviderType.DEEPSEEK_R1])
    ...
    def update_model_performance(self, provider: AIProviderType, quality_score: float):
        ...
        if len(self.model_performance_history[provider_name]) > 100:
            self.model_performance_history[provider_name] = self.model_performance_history[provider_name][-100:]
        
        # Update model config with recent performance
        if len(self.model_performance_history[provider_name]) >= 10:
            avg_performance = sum(self.model_performance_history[provider_name][-10:]) / 10
            self.model_configs[provider].quality_score = (
                self.model_configs[provider].quality_score * 0.7 + avg_performance * 0.3
            )
    
    def should_switch_model(self, current_provider: AIProviderType, current_quality: float) -> Optional[AIProviderType]:
        ...
        if current_quality < quality_threshold:
            ...
            if optimal_model != current_provider:
                self.logger.info(f"Switching from {current_provider.value} to {optimal_model.value} due to poor performance")
    ...
    def get_token_optimization_settings(self) -> Dict[str, Any]:
        ...
        return {
            "max_tokens": min(self.config.get("max_tokens", 1000), 1000),  # Cap at 1000 for cost control
            "context_optimization": self.config.get("context_window_optimization", True),
            "memory_optimization": self.config.get("conversation_memory_optimization", True),
