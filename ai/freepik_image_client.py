...
class FreepikImageClient:
    ...
    def __init__(self, api_key: str = None):
        ...
        if self.api_key:
            ...
            logger.info(
                f"🔑 API Key present: {self.api_key[:10]}...{self.api_key[-4:]}"
            )
        else:
            logger.error("❌ Freepik Image Client initialization FAILED - NO API KEY")
            logger.error(
                "🔧 Please set FREEPIK_API_KEY in Railway environment variables"
            )
    ...
    async def generate_image(
        ...
        try:
            ...
            logger.info(
                f"🚀 Making Freepik API request to: {self.base_url}/ai/text-to-image"
            )
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(
                        f"❌ Freepik API Error {response.status}: {error_text}"
                    )
                    ...
                    if response.status == 401:
                        ...
                        logger.error(
                            f"🔑 API Key used: {self.api_key[:10]}...{self.api_key[-4:]}"
                        )
                        logger.error(
                            "🔧 Check your Freepik API key at: https://www.freepik.com/developers/dashboard/api-key"
                        )
                        logger.error(
                            "⚙️  Make sure FREEPIK_API_KEY is set in Railway environment variables"
                        )
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await self.generate_image(
                prompt="A simple test image of a blue star", user_id=0
            )
            ...
            if test_result.get("success"):
                ...
                logger.error(
                    f"❌ Freepik API connection test failed: {test_result.get('error')}"
                )
                return {
                    "success": False,
    ...
    def __del__(self):
        ...
        if hasattr(self, "session") and self.session and not self.session.closed:
            ...
            logger.warning(
                "⚠️  Freepik session not properly closed - use async close() method"
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if client.is_available():
            ...
            print(
                "❌ Client not available - check FREEPIK_API_KEY environment variable"
            )
