...
class AIErrorHandler:
    ...
    def _initialize_providers(self):
        ...
        providers = {
            "openrouter": {
                ...
                "models": ["anthropic/claude-3-haiku", "meta-llama/llama-3.1-8b-instruct:free"],
                "priority": 1
            },
            "openrouter_backup": {
    ...
    def classify_error(self, error_message: str, status_code: int = None) -> AIErrorType:
        """Classify error type from error message and status code"""
        error_lower = error_message.lower()
        
        if status_code == 402 or "insufficient credits" in error_lower or "never purchased credits" in error_lower:
            return AIErrorType.INSUFFICIENT_CREDITS
        elif status_code == 429 or "rate limit" in error_lower:
            return AIErrorType.RATE_LIMITED
        elif status_code == 401 or "invalid api key" in error_lower or "unauthorized" in error_lower:
            return AIErrorType.INVALID_API_KEY
        elif "quota exceeded" in error_lower:
            return AIErrorType.QUOTA_EXCEEDED
        elif "timeout" in error_lower:
            return AIErrorType.TIMEOUT
        elif "model" in error_lower and ("unavailable" in error_lower or "not found" in error_lower):
    ...
    def handle_error(self, provider: str, error_message: str, status_code: int = None) -> Dict[str, Any]:
        ...
        if error_type == AIErrorType.INSUFFICIENT_CREDITS:
            ...
            logger.error(f"🚫 Provider {provider} has insufficient credits, backing off for 24 hours")
            ...
            backoff_time = min(self.base_backoff * (2 ** state["failure_count"]), self.max_backoff)
            state["backoff_until"] = current_time + backoff_time
            logger.warning(f"⏳ Provider {provider} rate limited, backing off for {backoff_time:.1f}s")
            return {"action": "fallback", "reason": "Rate limited", "backoff": backoff_time}
            ...
            logger.error(f"🔑 Provider {provider} has invalid API key, backing off for 1 hour")
            ...
            if state["failure_count"] >= self.max_retries:
                ...
                backoff_time = min(self.base_backoff * (2 ** state["failure_count"]), self.max_backoff)
                state["backoff_until"] = current_time + backoff_time
                logger.warning(f"⚠️ Provider {provider} failed {state['failure_count']} times, backing off")
                return {"action": "fallback", "reason": f"Multiple failures ({error_type.value})", "backoff": backoff_time}
            else:
                logger.warning(f"🔄 Provider {provider} error, will retry ({state['failure_count']}/{self.max_retries})")
                return {"action": "retry", "delay": self.base_backoff * state["failure_count"]}
    ...
    def get_next_provider(self, exclude: List[str] = None) -> Optional[str]:
        ...
        for provider in self.fallback_providers:
            ...
            if (state["last_failure"] and 
                current_time - state["last_failure"] > self.reset_threshold):
    ...
    def get_provider_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for provider, state in self.provider_states.items():
            is_available = (
                not state["backoff_until"] or 
                current_time >= state["backoff_until"]
            )
            ...
            status[provider] = {
                ...
                "backoff_remaining": max(0, (state["backoff_until"] or 0) - current_time)
            }
