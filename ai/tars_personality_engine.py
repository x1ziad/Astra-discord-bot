...
class TARSPersonalityCore:
    ...
    def adjust_humor_setting(self, new_level: int) -> str:
        ...
        if self.humor_setting == 0:
            ...
            return (
                f"Humor reduced to {self.humor_setting}%. More serious mode activated."
            )

    def get_personality_response(
        self, context: str, user_input: str, response_type: str = "general"
    ) -> Dict[str, Any]:
    ...
    def determine_response_mode(
        ...
        if any(
            word in input_lower
            for word in ["analyze", "calculate", "logic", "data", "statistics"]
        ):
            ...
            word in input_lower
            for word in ["joke", "funny", "humor", "laugh", "amusing"]
        ):
            return TARSPersonalityMode.WITTY

        elif any(
            word in input_lower
            for word in ["honest", "truth", "really", "actually", "serious"]
        ):
            return TARSPersonalityMode.HONEST

        elif any(
            word in input_lower
            for word in ["problem", "solve", "fix", "issue", "challenge"]
        ):
    ...
    def process_user_feedback(self, user_id: int, feedback: str, response_rating: int):
        ...
        if "too funny" in feedback.lower() or "less humor" in feedback.lower():
            ...
            prefs["preferred_humor_level"] = min(
                100, prefs["preferred_humor_level"] + 10
            )
...
def get_tars_response(
    context: str, user_input: str, user_id: int = None
) -> Dict[str, Any]:
