...
class OptimizedImageGenerator:
    ...
    def __init__(self, api_key: str = None):
        ...
        if self.api_key:
            logger.info(
                f"🔑 API Key configured: {self.api_key[:10]}...{self.api_key[-6:]}"
            )
        else:
            ...
            for attempt in range(max_retries):
                try:
                    async with session.post(
                        ...
                        logger.info(
                            f"📡 API Response: {response.status} (attempt {attempt + 1})"
                        )
                        ...
                        if response.status == 200:
                            ...
                            if data.get("data") and len(data["data"]) > 0:
                                ...
                                if download_image:
                                    ...
                                    if image_bytes:
                                        ...
                                        logger.info(
                                            f"✅ Image ready: {len(image_bytes)} bytes"
                                        )
                                    else:
                                        logger.warning(
                                            "⚠️ Failed to download image, URL only"
                                        )
                                ...
                                logger.info(
                                    f"🎉 Image generated successfully in {response_time:.1f}ms"
                                )
                                return result
                            ...
                            error_text = await response.text()
                            logger.warning(
                                f"🔄 Server error {response.status}, retrying... {error_text}"
                            )
                            if attempt < max_retries - 1:
                                await asyncio.sleep(
                                    (attempt + 1) * 2
    ...
    async def clear_cache(self, older_than_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"🧹 Cleared {cleared} cache files older than {older_than_hours} hours"
            )
...
async def generate_image_fast(
    ...
    return await generator.generate_image_optimized(
        prompt=prompt, user_id=user_id, size=size, download_image=download_bytes
    )
...
if __name__ == "__main__":
    ...
    async def test_optimized():
        ...
        result = await generator.generate_image_optimized(
            "a simple test image of a cute robot", user_id=12345, download_image=True
        )
