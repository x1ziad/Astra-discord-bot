...
class MLUserAnalyzer:
    ...
    def _extract_behavioral_features(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        features["communication_patterns"] = {
            ...
            "emoji_per_message": (
                emoji_usage / len(conversations) if conversations else 0.0
            ),
            "question_ratio": (
                question_count / len(conversations) if conversations else 0.0
            ),
        }

        # Engagement responsiveness
        engagement_scores = [conv[4] for conv in conversations if conv[4] is not None]
        features["engagement_responsiveness"] = (
    ...
    async def cluster_users(self, min_interactions: int = 10) -> Dict[int, int]:
        ...
        try:
            ...
            eligible_users = [
                ...
                if profile.interaction_frequency
                >= min_interactions / 30  # At least min_interactions per month
            ]
            ...
            for user_id in eligible_users:
                ...
                features.extend(
                    [profile.activity_patterns.get(str(h), 0.0) for h in range(24)]
                )
            ...
            logger.info(
                f"✅ Clustered {len(eligible_users)} users into {n_clusters} groups"
            )
    ...
    def predict_engagement_success(
        self, user_id: int, current_hour: int, topic: str = None
    ) -> float:
        ...
        try:
            ...
            time_multiplier = 1.0 + (
                hour_activity * 2
            )  # Boost if it's their active time
    ...
    def get_optimal_engagement_strategy(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get top topics
            top_topics = sorted(
                profile.topic_preferences.items(), key=lambda x: x[1], reverse=True
            )[:3]

            preferred_topics = (
                [topic for topic, _ in top_topics] if top_topics else ["general"]
            )
            ...
            return {
                ...
                "sentiment_approach": (
                    "positive" if profile.sentiment_baseline > 0.6 else "supportive"
                ),
            }
    ...
    async def update_user_feedback(
        self, user_id: int, interaction_success: bool, context: Dict[str, Any]
    ):
        ...
        try:
            ...
            if interaction_success:
                profile.engagement_responsiveness = min(
                    1.0, profile.engagement_responsiveness + feedback_weight
                )
            else:
                profile.engagement_responsiveness = max(
                    0.0, profile.engagement_responsiveness - feedback_weight
                )
            ...
            logger.debug(
                f"Updated feedback for user {user_id}: success={interaction_success}"
            )
    ...
    async def get_analytics_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            peak_hours = sorted(
                hourly_totals.items(), key=lambda x: x[1], reverse=True
            )[:3]
