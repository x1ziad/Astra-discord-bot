...
class ConversationMemory:
    ...
    def _calculate_importance(self, content: str, role: str, metadata: Dict[str, Any]) -> float:
        ...
        # Important keywords
        important_keywords = [
            "help", "problem", "issue", "error", "confused", "explain", 
            "stellaris", "strategy", "important", "remember", "always"
        ]
        keyword_matches = sum(1 for keyword in important_keywords if keyword.lower() in content.lower())
    ...
    def get_relevant_context(self, current_message: str, max_context: int = 10) -> List[Dict[str, Any]]:
...
- Share insights about optimal strategies and builds"""
    ...
    def build_dynamic_prompt(self, 
        ...
        interaction_guidance = self._build_interaction_guidance(current_message, conversation_memory)
        base_prompt += f"\n\n{interaction_guidance}"
        
        # Add response style guidance
        style_guidance = self._build_style_guidance(conversation_memory, current_message)
    ...
    def _analyze_context_type(self, memory: ConversationMemory, message: str) -> str:
        ...
        technical_keywords = ["how", "why", "explain", "stellaris", "strategy", "mechanics", "build", "optimize"]
        if any(keyword in message_lower for keyword in technical_keywords):
            if any(game_term in message_lower for game_term in ["stellaris", "empire", "species", "fleet"]):
                return "strategic"
            else:
                return "technical"
        
        # Check for emotional/support content  
        support_keywords = ["help", "confused", "frustrated", "problem", "issue", "stuck", "don't understand"]
    ...
    def _build_memory_context(self, memory: ConversationMemory) -> str:
        ...
        if memory.important_messages:
            ...
            if important_points:
                context_parts.append(f"Important previous points: {' | '.join(important_points)}")
        
        return "; ".join(context_parts) if context_parts else ""
    
    def _build_interaction_guidance(self, message: str, memory: ConversationMemory) -> str:
        """Build guidance for this specific interaction"""
        guidance = "For this response:"
        
        # Message length adaptation
        if len(message) < 30:
            guidance += " User sent a brief message, so keep response concise but helpful."
    ...
    def _build_style_guidance(self, memory: ConversationMemory, message: str) -> str:
        ...
        if memory.messages:
            recent_user_messages = [msg for msg in list(memory.messages)[-5:] if msg["role"] == "user"]
            ...
            if recent_user_messages:
                ...
                if user_uses_emojis:
                    ...
                    style_elements.append("user doesn't use emojis, keep response clean")
                
                # Check formality level
                avg_length = sum(len(msg["content"]) for msg in recent_user_messages) / len(recent_user_messages)
                if avg_length > 100:
...
class OptimizedAIEngine:
        ...
        try:
            if UNIVERSAL_AI_AVAILABLE:
                ...
                if self.ai_client.is_available():
                    logger.info(f"✅ Optimized AI client initialized with {self.current_model.value}")
                else:
    ...
    async def process_conversation(self,
        ...
        try:
            ...
            response = await self._generate_optimized_response(message, memory, user_profile)
            ...
            logger.debug(f"Generated response for user {user_id} in {response_time:.2f}ms")
            return response
    ...
    def _extract_themes(self, message: str) -> Set[str]:
        ...
        theme_keywords = {
            "stellaris": ["stellaris", "empire", "species", "galactic", "federation", "fleet"],
            "space": ["space", "cosmos", "universe", "star", "planet", "galaxy", "astronomy"],
            "gaming": ["game", "gaming", "play", "strategy", "multiplayer", "build"],
            "science": ["science", "research", "theory", "experiment", "discovery"],
            "technology": ["tech", "technology", "ai", "computer", "software", "algorithm"],
            "help": ["help", "assistance", "support", "guide", "tutorial", "how to"],
            "strategy": ["strategy", "tactics", "plan", "optimize", "efficient", "best"]
        }
    ...
    def _generate_cache_key(self, message: str, user_id: int, memory: ConversationMemory) -> str:
    ...
    async def _generate_optimized_response(self, 
                                         message: str,
        ...
        try:
            ...
            system_prompt = self.prompt_engine.build_dynamic_prompt(memory, message, user_profile)
            ...
            if response.tokens_used:
                self.token_usage_tracking[self.current_model.value] += response.tokens_used
            
            # Post-process response
            processed_response = self._post_process_response(response.content, memory, user_profile)
            
            # Evaluate response quality
            quality = self._evaluate_response_quality(message, processed_response, memory)
    ...
    def _post_process_response(self, 
        ...
        if memory.messages:
            ...
            if user_uses_emojis and not any(ord(char) > 127 for char in response):
                if any(theme in memory.conversation_themes for theme in ["space", "stellaris"]):
                    if len(response) < 100:
                        response += " 🌌"
    ...
    def _evaluate_response_quality(self, 
                                 message: str,
                                 response: str,
                                 memory: ConversationMemory) -> ResponseQuality:
    ...
    def _evaluate_coherence(self, message: str, response: str) -> float:
        ...
        if "?" in message and any(word in response.lower() for word in ["yes", "no", "because", "due to", "result"]):
            score += 0.2
    ...
    def _evaluate_relevance(self, message: str, response: str, memory: ConversationMemory) -> float:
    ...
    def _evaluate_personalization(self, response: str, memory: ConversationMemory) -> float:
        ...
        if memory.messages:
            ...
            if user_messages:
                ...
                avg_user_length = sum(len(msg["content"]) for msg in user_messages[-3:]) / min(3, len(user_messages))
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        for metric_name, values in self.quality_metrics.items():
            if values:
                ...
                metrics[f"recent_{metric_name}"] = values[-10:] if len(values) >= 10 else values
        
        # Token usage
        metrics["token_usage"] = dict(self.token_usage_tracking)
        
        # Cache performance
        cache_hits = sum(1 for data in self.response_cache.values() if time.time() - data["timestamp"] < self.cache_ttl)
        metrics["cache_hit_rate"] = cache_hits / max(1, len(self.response_cache)) * 100
        
        # User engagement
        metrics["active_users"] = len(self.conversation_memories)
        metrics["total_conversations"] = sum(len(memory.messages) for memory in self.conversation_memories.values())
    ...
    async def optimize_model_selection(self) -> str:
        ...
        current_quality = sum(self.quality_metrics["response_quality"][-10:]) / min(10, len(self.quality_metrics["response_quality"]))
        
        # If current model is performing well, stick with it
        if current_quality > 0.8:
            logger.info(f"Current model {self.current_model.value} performing well (quality: {current_quality:.2f})")
            return self.current_model.value
        
        # If performance is poor, consider switching
        if current_quality < 0.6:
            logger.warning(f"Current model {self.current_model.value} underperforming (quality: {current_quality:.2f})")
    ...
    async def cleanup_memory(self):
        ...
        for user_id in users_to_cleanup:
            ...
            if memory.important_messages:
                ...
                archived_important = memory.important_messages[-5:]  # Keep last 5 important messages
...
if __name__ == "__main__":
    ...
    async def test_optimized_engine():
        ...
        test_message = "Hello! Can you help me understand Stellaris fleet composition strategies?"
