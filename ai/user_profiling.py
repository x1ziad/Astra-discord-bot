...
class UserProfileManager:
    ...
    async def get_user_profile(self, user_id: int, username: str = None) -> UserPersonality:
    ...
    async def _load_profile_from_db(self, user_id: int, username: str = None) -> UserPersonality:
        ...
        try:
            ...
            return UserPersonality(user_id=user_id, username=username or f"User{user_id}")
    ...
    async def analyze_message(self, user_id: int, message: str, username: str = None) -> Dict[str, Any]:
    ...
    async def _extract_topics(self, message: str) -> List[str]:
        ...
        topic_keywords = {
            "space": ["space", "universe", "cosmos", "galaxy", "star", "planet", "asteroid", "nebula"],
            "stellaris": ["stellaris", "empire", "colony", "fleet", "technology", "research"],
            "science": ["science", "research", "experiment", "theory", "discovery"],
            "technology": ["tech", "technology", "computer", "software", "AI", "machine"],
            "gaming": ["game", "gaming", "play", "player", "level", "achievement"],
            "programming": ["code", "programming", "python", "javascript", "debug", "function"],
            "personal": ["i", "me", "my", "myself", "feel", "think", "believe"],
            "questions": ["how", "what", "why", "when", "where", "can you", "could you"],
            "emotions": ["happy", "sad", "excited", "frustrated", "confused", "amazing"]
        }
    ...
    async def _analyze_sentiment(self, message: str) -> float:
        ...
        positive_words = [
            "good", "great", "awesome", "amazing", "excellent", "fantastic", "love", "like",
            "happy", "excited", "wonderful", "perfect", "brilliant", "outstanding"
        ]
        
        negative_words = [
            "bad", "terrible", "awful", "hate", "dislike", "sad", "angry", "frustrated",
            "confused", "difficult", "problem", "issue", "wrong", "error"
        ]
    ...
    async def _analyze_formality(self, message: str) -> float:
        ...
        formal_indicators = [
            "please", "thank you", "could you", "would you", "may i", "excuse me",
            "i would like", "i appreciate", "furthermore", "however", "therefore"
        ]
        
        casual_indicators = [
            "hey", "hi", "yo", "sup", "gonna", "wanna", "kinda", "sorta",
            "yeah", "yep", "nah", "lol", "omg", "btw", "tbh"
        ]
    ...
    async def _analyze_technical_content(self, message: str) -> float:
        ...
        technical_terms = [
            "algorithm", "database", "server", "API", "function", "variable", "loop",
            "array", "object", "class", "method", "parameter", "return", "import",
            "library", "framework", "deployment", "configuration", "optimization"
        ]
    ...
    async def _store_conversation(self, user_id: int, message: str, analysis: Dict[str, Any]):
    ...
    async def _update_user_profile(self, user_id: int, analysis: Dict[str, Any], username: str = None):
        ...
        profile.formal_preference = (profile.formal_preference * (1 - weight) + 
                                   analysis["formality"] * weight)
        
        profile.technical_interest = (profile.technical_interest * (1 - weight) + 
                                    analysis["technical_content"] * weight)
        
        profile.emoji_usage = (profile.emoji_usage * (1 - weight) + 
                              (1.0 if analysis["contains_emoji"] else 0.0) * weight)
        
        # Update average message length
        profile.avg_message_length = ((profile.avg_message_length * (profile.interaction_count - 1) + 
                                     analysis["length"]) / profile.interaction_count)
    ...
    async def get_personalized_context(self, user_id: int) -> Dict[str, Any]:
        ...
        context = {
            "user_personality": {
                ...
                "communication_style": "formal" if profile.formal_preference > 0.6 else "casual",
                "typical_message_length": "long" if profile.avg_message_length > 100 else "short",
