...
class InviteView(discord.ui.View):
    ...
    def __init__(self, client_id: str):
        ...
        self.recommended_url = generate_bot_invite_url(
            client_id, get_recommended_permissions()
        )
        self.minimal_url = generate_bot_invite_url(client_id, get_minimal_permissions())

    @discord.ui.button(
        label="Full Permissions", style=discord.ButtonStyle.green, emoji="🌟"
    )
    async def full_permissions(
    ...
    @discord.ui.button(
        label="Recommended", style=discord.ButtonStyle.blurple, emoji="⭐"
    )
    async def recommended_permissions(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        embed = discord.Embed(
...
class BotSetup(commands.Cog):
    ...
    @app_commands.command(
        name="invite", description="Get invitation links to add Astra to other servers"
    )
    ...
    async def invite_command(
        self, interaction: discord.Interaction, permission_level: str = "interactive"
    ):
        ...
        if permission_level == "interactive":
            ...
            embed.set_footer(
                text="Click a button below to get the appropriate invitation link"
            )
            ...
            embed.add_field(name="✅ Features Included:", value=features, inline=False)
        ...
        self.logger.info(
            f"Invitation link ({permission_level}) requested by {interaction.user} in {interaction.guild}"
        )

    @app_commands.command(
        name="setup", description="Comprehensive setup guide for server administrators"
    )
    async def setup_command(self, interaction: discord.Interaction):
        ...
        if not interaction.user.guild_permissions.manage_guild:
            await interaction.response.send_message(
                "❌ This command requires 'Manage Server' permission.", ephemeral=True
            )
            return
        ...
        test_button = discord.ui.Button(
            label="Test Bot", style=discord.ButtonStyle.green, emoji="🧪"
        )

        async def test_callback(button_interaction):
            test_embed = discord.Embed(
                title="🧪 Bot Test Results", color=unified_config.get_color("success")
            )
            ...
            test_embed.add_field(
                name="📊 Quick Stats",
                value=f"Latency: {self.bot.latency*1000:.0f}ms\nGuilds: {len(self.bot.guilds)}\nUptime: {self.bot.stats.get_uptime()}",
                inline=False,
            )
            await button_interaction.response.send_message(
                embed=test_embed, ephemeral=True
            )
        ...
        self.logger.info(
            f"Setup guide requested by {interaction.user} in {interaction.guild.name}"
        )
    ...
    async def diagnostics_command(self, interaction: discord.Interaction):
        ...
        if bot_member:
            ...
            diagnostics["permissions"] = (
                ...
                if all(critical_perms.values())
                else "⚠️ Missing some permissions"
            )
            diagnostics["permission_details"] = critical_perms

        # Extension health
        healthy_extensions = sum(
            1 for health in self.bot.extension_health.values() if health
        )
        total_extensions = len(self.bot.extension_health)
        diagnostics["extensions"] = (
            f"✅ {healthy_extensions}/{total_extensions} healthy"
        )
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY", timeout=5
                ) as resp:
                    diagnostics["nasa_api"] = (
                        "✅ Accessible"
                        if resp.status == 200
                        else f"⚠️ Status {resp.status}"
                    )
        except Exception as e:
        ...
        embed.add_field(
            name="🔐 Permissions", value=diagnostics["permissions"], inline=True
        )
        ...
        if "permission_details" in diagnostics:
            ...
            embed.add_field(
                name="📋 Detailed Permissions", value=perm_status, inline=False
            )
        ...
        self.logger.info(
            f"Diagnostics run by {interaction.user} in {interaction.guild.name}"
        )
