...
def verify_error_fixes():
    ...
    try:
        ...
        if "use_application_commands" in content:
            ...
            results["potential_issues"].append(
                "⚠️ use_application_commands not found - may need manual verification"
            )
            print("  ⚠️ use_application_commands not found - manual check needed")

    except Exception as e:
        results["potential_issues"].append(
            f"❌ Error checking bot_setup_enhanced.py: {e}"
        )
        results["verification_success"] = False
    ...
    try:
        ...
        if "v.timestamp" in content:
            results["potential_issues"].append(
                "❌ Direct v.timestamp access still found"
            )
            results["verification_success"] = False
        ...
        if "datetime.fromisoformat(v['timestamp'])" in content:
            ...
            results["potential_issues"].append(
                "⚠️ Dict timestamp parsing not found - may need manual verification"
            )
            print("  ⚠️ Dict timestamp parsing not found - manual check needed")

    except Exception as e:
        results["potential_issues"].append(
            f"❌ Error checking security_commands.py: {e}"
        )
        results["verification_success"] = False
        print(f"  ❌ Error reading file: {e}")
            ...
            if import_name == "discord.Permissions":
                ...
                if hasattr(perms, "use_application_commands"):
                    results["import_tests"].append(
                        f"✅ {description} - use_application_commands available"
                    )
                    print(
                        f"  ✅ {description} - use_application_commands permission exists"
                    )
                else:
                    results["import_tests"].append(
                        f"⚠️ {description} - use_application_commands not found"
                    )
                    print(
                        f"  ⚠️ {description} - use_application_commands permission not found"
                    )
                ...
                results["import_tests"].append(
                    f"✅ {description} - imports successfully"
                )
    ...
    for pattern, description in potential_problem_patterns:
        ...
        if found_files:
            results["potential_issues"].append(
                f"⚠️ {description} found in: {', '.join(found_files)}"
            )
