...
def log_command_usage(command_name: str, user_id: int, guild_id: int = None):
    ...
    logger.info(f"📊 Command '{command_name}' used by user {user_id}{guild_info} at {timestamp}")
...
def log_api_request(api_name: str, endpoint: str, response_time: float, status_code: int = None):
